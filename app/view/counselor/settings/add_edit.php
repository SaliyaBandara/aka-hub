...
<div id="sidebar-active" class="hideScrollbar">
    ...
    <div class="my-2 mx-2">
        ...
        <form action="" method="post" class="form">
            ...
            <?php
            foreach ($data["counselor_profile_template"] as $key => $value) {
                if (isset($value["skip"]) && $value["skip"] == true)
                    continue;
            ?>
                <div class="mb-1 form-group">
                    <label for="name" class="form-label"><?= $value["label"] ?></label>
                    <input type="<?= $value["type"] ?>" id="<?= $key ?>" name="<?= $key ?>" placeholder="Enter <?= $value["label"] ?>" value="<?= $data["counselor_profile"][$key] ?>" <?= $value["validation"] == "required" ? "data-validation='required'" : "" ?> class="form-control">
                </div>
            <?php
            }
            ?>
            ...
            <div class="mt-1-5 form-group">
                <a href="<?= BASE_URL ?>/counselorSettings" class="btn btn-info">Back</a>
