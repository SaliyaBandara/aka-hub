...
<div id="sidebar-active" class="hideScrollbar">
    ...
    <div class="my-2 mx-2">
        <h3 class="text-muted">Edit Course Details</h3>
        <form action="" method="post" class="form">
            ...
            <div class="mb-1">
                ...
                <div action="/uploadFiles/img/profile_img" data-name="cover_img" data-maxFiles="1" class="dropzone imgDropZone"></div>
            </div>

            <div class="mb-1">
                <div class="table-responsive">
                    <table <?= ($data["course"]['id'] == 0 ? 'style="display: none;"' : "") ?> data-name="cover_img" class="table table-custom2 custom-table table-borderless image-preview-table sortableTable" width="100%" cellspacing="0">
                        <thead class="cent">
                            <tr>
                        ...
                        <tbody class="ui-sortable">
                            <?php
                            if ($data["course"]['id'] != 0 && !empty($data["course"]['cover_img'])) {
                                $img_src = USER_IMG_PATH . $data["course"]["cover_img"];
                            ?>
                                <tr class='ui-sortable-handle'>
                                    <td>
                                        <div class='preview-img preview-img-small' data-filename='<?= $data["course"]['cover_img'] ?>' data-fancybox='group' href='<?= $img_src ?>'>
            ...
            $('.dropzone.imgDropZone').each(function() {
                ...
                let myDropzone = new Dropzone(this, {
                    ...
                    init: function() {
                        this.on('addedfile', function(file) {
                            if (++table.find("tbody tr").length > maxFiles) {
                                alertUser("danger", `Only ${maxFiles} image(s) are allowed`)
                    ...
                    success: function(file, response) {
                        ...
                        if (++table.find("tbody tr").length > maxFiles)
                            return (alertUser("danger", `Maximum number of images reached`));
