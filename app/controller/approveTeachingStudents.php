...
class ApproveTeachingStudents extends Controller
{
    ...
    public function acceptRole($id, $role)
    {
        ...
        if ($result) {
            ...
            $status = "606";
            $this->model("createModel")->createLogEntry($action, $status);
            $this->model("createModel")->notification(7, 0, $id, "Promoted to Teaching Student", "You Have Been Promoted to Teaching Student and Features", 7);
    ...
    public function declineRole($id = 0, $role = 0)
    {
        ...
        if ($result) {
            ...
            $status = "200";
            $this->model("createModel")->createLogEntry($action, $status);
            $this->model("createModel")->notification(7, 0, $id, "Promotion to Teaching Student Rejected", "You Have Been Promoted to Club Representative Role and Features", 7);
    ...
    public function removeAccess($id = 0, $role = 0)
    {
        ...
        if ($result1) {
            ...
            $status = "607";
            $this->model("createModel")->createLogEntry($action, $status);
            $this->model("createModel")->notification(7, 0, $id, "Teaching Student Permissions Revoked", "Sorry , Your Teaching Student Permissions and Features Revoked", 7);
            die(json_encode(array("status" => "200", "desc" => "Denying Successfull")));
        } else {
            die(json_encode(array("status" => "400", "desc" => "Denying unsuccessfull")));
        }
